...
import { Sparkles, Layers, Cpu, Compass, Bot, Waves } from "lucide-react";
...
function ModelIcon({ model, className }: { model: string; className?: string }) {
  ...
  if (hasError || !icons[model]) {
    switch (model) {
      case "ORCA":
        return <Waves className={cn("size-3.5 text-cyan-400", className)} />;
      case "Gemini 3.5 Flash":
        return <Sparkles className={cn("size-3.5 text-sky-500", className)} />;
      case "Opus 4.8":
        return <Layers className={cn("size-3.5 text-amber-500", className)} />;
      case "Composer 2.5":
        return <Cpu className={cn("size-3.5 text-purple-500", className)} />;
      case "GLM 5.2":
        return <Compass className={cn("size-3.5 text-emerald-500", className)} />;
      default:
        return <Bot className={cn("size-3.5 text-slate-700", className)} />;
    }
  }
...
export const PromptInput = React.forwardRef<HTMLDivElement, PromptInputProps>(
  (
    ...
    const [selectedModel, setSelectedModel] = useState("ORCA");
    ...
    return (
      <>
        ...
        <div
          ...
          style={{
            maxWidth: expanded ? (className?.includes("max-w-full") ? "100%" : 480) : (className?.includes("max-w-full") ? "min(360px, 100%)" : 320),
        ...
        >
          ...
          >
            ...
            >
              <span className="truncate whitespace-nowrap block w-full">
                {placeholder}
              </span>
            </button>
            ...
            <div
              className={cn(
                "absolute bottom-2 left-3 right-12 z-[10] flex items-center gap-2 transition-all duration-300 ease-[cubic-bezier(0.175,0.885,0.32,1.275)]",
